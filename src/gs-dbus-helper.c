			 ...
			 const gchar	   *ident)
{
	...
	g_notification_add_button_with_target (n, _("Find in Software"), "app.install-resources", "(s^asss)", mode_string, resources, "", ident);
	g_notification_set_default_action_and_target (n, "app.install-resources", "(s^asss)", mode_string, resources, "", ident);
		   ...
		   GDBusMethodInvocation *invocation)
{
	...
	g_action_group_activate_action (G_ACTION_GROUP (app), "install-resources",
	                                g_variant_new ("(s^asss)", mode_string, resources, startup_id, ident ? ident : ""));
}
